...
export default function CreateCampaign() {
  ...
  const [titleSameAsBrand, setTitleSameAsBrand] = useState(false);
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [campaignType, setCampaignType] = useState('awareness');

  // Auto-populate title when checkbox is checked
  const handleTitleSameToggle = (checked: boolean) => {
    setTitleSameAsBrand(checked);
    if (checked && brandName.trim()) {
      setTitle(brandName.trim());
    }
  };

  // Auto-populate description when brand name changes (if description is empty or was auto-generated)
  const [descAutoGenerated, setDescAutoGenerated] = useState(false);
  const generateDefaultDescription = (name: string) => {
    return `Share your honest experience with ${name}! We'd love to hear what you think about our products and services. Post your genuine review on social media and earn exciting rewards. Whether you loved it or have feedback to share — every voice matters.`;
  };

  const handleBrandNameChange = (name: string) => {
    setBrandName(name);
    if (titleSameAsBrand) setTitle(name);
    if (!description.trim() || descAutoGenerated) {
      if (name.trim()) {
        setDescription(generateDefaultDescription(name.trim()));
        ...
        {step === 0 && (
          <div>
            <div className="form-group">
              ...
              <input className="input" placeholder="e.g. Acme Corp" value={brandName}
                onChange={(e) => handleBrandNameChange(e.target.value)} />
              <p style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', marginTop: '0.25rem' }}>
                Your company or brand name — used in the campaign page and embed widgets
              </p>
            </div>
            <div className="form-group">
              <label className="label">Brand Website URL</label>
              <input className="input" placeholder="e.g. https://acmecorp.com" value={brandWebsite}
                onChange={(e) => setBrandWebsite(e.target.value)} />
            </div>
            <div className="form-group">
              <label className="label">Campaign Title *</label>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.82rem', color: 'var(--color-text-secondary)', cursor: 'pointer' }}>
                  <input type="checkbox" checked={titleSameAsBrand}
                    onChange={(e) => handleTitleSameToggle(e.target.checked)} />
                  Same as brand name
                </label>
              </div>
              <input className="input" placeholder="e.g. Summer Product Launch"
                value={title} onChange={(e) => { setTitle(e.target.value); if (titleSameAsBrand && e.target.value !== brandName) setTitleSameAsBrand(false); }}
                disabled={titleSameAsBrand} style={{ opacity: titleSameAsBrand ? 0.6 : 1 }} />
            </div>
            <div className="form-group">
              <label className="label">Description *</label>
              <textarea className="input" placeholder="Describe what advocates should know..."
                value={description} onChange={(e) => { setDescription(e.target.value); setDescAutoGenerated(false); }} style={{ minHeight: 120 }} />
              {descAutoGenerated && (
                <p style={{ fontSize: '0.72rem', color: 'var(--color-primary-light)', marginTop: '0.25rem' }}>
                  ✨ Auto-generated from your brand name. Feel free to edit!
                </p>
              )}
            </div>
            <div className="form-group">
